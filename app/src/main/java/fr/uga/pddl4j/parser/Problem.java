...
public class Problem implements Serializable {
    ...
    //Get initial facts by based Predicate
    public Exp getInitialFactsByBasePredicate( String basePredicate )
    {
        Exp targetExp = null;

        for( Exp exp : this.initialFacts )
        {
            String tempBasePredicate = InterventionStringUtil.getTheBasePredicateFromExp( exp );
            if( tempBasePredicate != null &&
                    tempBasePredicate.toLowerCase().trim().equals( basePredicate.trim() ) )
            {
                targetExp = exp;
                break;
            }
